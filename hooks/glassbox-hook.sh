...
# Fastest runner first: a global install, then a built clone, then npx.
if command -v glassbox >/dev/null 2>&1; then
  set -- glassbox
elif [ -n "${CLAUDE_PLUGIN_ROOT:-}" ] && [ -f "$CLAUDE_PLUGIN_ROOT/dist/cli/index.js" ]; then
